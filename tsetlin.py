...
class Tsetlin:
    ...
    def reward(self, state):
        if state in [1, self.n + 1, 2*self.n + 1, 3*self.n + 1, 4*self.n + 1, 5*self.n + 1]:
            return state
        else:
            state -= 1
            return state
    ...
    def penalize(self, state):
        end_states = [self.n, 2*self.n, 3*self.n, 4*self.n, 5*self.n, 6*self.n]
    ...
    def tsetlin_automaton(self, batch, cutoff):
    ...
    def main(self):
        ...
        for i in range(100):
            accuracy, delay, learned_floor = self.tsetlin_automaton(10000, 1000)
